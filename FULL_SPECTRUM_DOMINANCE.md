...
>
> **三轮复核（2026-08-20，冻结二进制 artifact 测量）**：
>
> | 套件 | deepseek | claude-code |
> | --- | --- | --- |
> | Kimi（16 题 × 3 轮，完整） | **48/48（100%）**，0 审批，9.2K tok/成功，26.5s | 46/48（96%），1.8 审批，7.3K tok，66s |
> | v4-pro（60 题 × 3 轮，部分） | 54/55 有效运行（98%），0 审批 | 数据无效（账户余额耗尽，全部 402） |
>
> 三轮复核结论：成功率（100% vs 96%）与零审批优势在多轮下稳定；claude-code 两次
> 超时（vs-014、vs-016，300s 上限）；token 成本优势在 Kimi 三轮中未复现（对手缓存
> 命中更好），耗时优势稳定（约 2.5×）。v4 全量对照因账户余额耗尽中断，充值后需重跑。
> 复核还抓出并修复了第三个 harness 级缺陷：Provider 偶发空完成会被沉默判定为失败，
> 已加"空回答追问一次"韧性（agent-executor，有测试）。
>
> **单轮预跑存档（16 题 × 1 轮 × 两模型族）**：
>
