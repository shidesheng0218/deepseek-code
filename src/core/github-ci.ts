...
export async function getGitHubCIStatus(_projectPath: string, commitSHA: string, execute: (args: string[]) => Promise<string>): Promise<{ currentCommit: GitHubWorkflowRun[]; staleCount: number; passed: boolean }> {
  ...
  const passed = currentCommit.length > 0 && currentCommit.every((run) => run.status === 'completed' && run.conclusion === 'success');
  return { currentCommit, staleCount: runs.filter((run) => run.headSha !== commitSHA).length, passed };
}
