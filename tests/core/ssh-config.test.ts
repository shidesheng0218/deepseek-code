...
describe('project SSH configuration', () => {
  test('loads only explicitly fingerprint-pinned hosts from .deepseek/ssh.json', async () => {
    ...
    await writeFile(join(root, '.deepseek', 'ssh.json'), JSON.stringify({ hosts: [{ id: 'prod', hostname: 'example.test', user: 'deploy', port: 22, fingerprint: 'SHA256:known', remotePath: '/home/deploy/deepseek-host' }] }));

    await expect(loadSSHProjectConfig(root)).resolves.toEqual({ hosts: [{ id: 'prod', hostname: 'example.test', user: 'deploy', port: 22, fingerprint: 'SHA256:known', remotePath: '/home/deploy/deepseek-host' }] });
  });
